...
public class WordsFlowView extends FrameLayout implements OnGlobalLayoutListener {

	private static final String TAG = "WordsFlowView";
	...
	public WordsFlowView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public WordsFlowView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public WordsFlowView(Context context) {
		super(context);
		init();
	}
	...
	public void feedKeywordsAll(String[] words) {
	...
	public void feedKeywordsRadom(String[] arr) {
	...
	private boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
			...
			for (int i = 0; i < size; i++) {
				...
				txt.setBackgroundResource(R.drawable.bg_flowword);// 设置点击效果
				// 获取文本长度
